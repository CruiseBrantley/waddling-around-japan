...
# We ensure the remote src folder exists, then copy all flat TypeScript files directly inside.
ssh "${PI_USER}@${PI_HOST}" "mkdir -p ${DEST_DIR}/src"
scp server/src/*.ts "${PI_USER}@${PI_HOST}:${DEST_DIR}/src/"
...
Write-Host "[BUILD] Rebuilding and Restarting on Pi..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Remote build/restart failed!" -ForegroundColor Red
    exit $LASTEXITCODE
}
